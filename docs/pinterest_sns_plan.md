...
ブログ記事1本から、SNS投稿を3〜4個切り出す。**Xでは、これを4つの独立した投稿ではなく1本のスレッド(ルート投稿+リプライの連鎖)として投稿する。**

1. **ルート投稿(フック、リンクなし)**: 記事の核心的な問いを1文で(例: 「Weighted blankets: placebo or does the research actually back this up?」)
2. **リプライ1(リスト投稿、リンクなし)**: 記事内の表や比較ポイントを箇条書きで再構成
3. **リプライ2(Before/After的投稿、リンクなし)**: 「よくある間違い→正しい選び方」の対比
4. **リプライ3(リンク付き投稿)**: 記事へのCTA、ピンと同じ説明文の短縮版。スレッドの最後に置く

`scripts/run_promotion.py`はmanifestの`tweets`配列を上から順にリプライ連結して投稿するので、プロモーターはこの順序(フック→リスト→Before/After→リンク付きCTA)を守ってmanifestを作ればよい。ルート投稿をリンクなしにするのは、Xが外部リンク付き投稿のリーチを抑制する傾向があるため、拡散の起点は必ずリンクなしにするという狙いがある。
