...
## SNS投稿(3〜4案、X用・1スレッドとして投稿する)
1. ルート投稿(フック、リンクなし):
2. リプライ1(リスト投稿、記事内の比較表・要点を再構成、リンクなし):
3. リプライ2(Before/After対比投稿、リンクなし):
4. リプライ3(リンク付きCTA投稿、リンクあり):
```

### SNS投稿はスレッド形式で作る(重要)
X投稿は3〜4件をそれぞれ独立した投稿として作るのではなく、**1本のスレッド**として作る。1件目をルート投稿、2件目以降はその直前の投稿へのリプライとして繋げる(`scripts/run_promotion.py`が`tweets`配列の順序どおりに自動でリプライ連結して投稿する)。

- ルート投稿(1件目)は必ずリンクなしのフック投稿にする。Xは外部リンクを含む投稿のリーチが抑制されやすいため、最初の投稿はリンクなしにして拡散を優先する
- リンク付きCTA投稿は基本的に最後(スレッドの末尾)に置く
- 同じ記事について毎週バラバラに複数投稿してフォロワーのタイムラインを占有するより、1スレッドにまとめる方がフォロワー体験として自然
